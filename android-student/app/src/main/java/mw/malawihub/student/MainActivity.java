...
public class MainActivity extends Activity {
    ...
    private void showBrandedSplash() {
        ...
        splash.setPadding(32,32,32,32); splash.setBackgroundColor(Color.WHITE);
        ImageView logo = new ImageView(this); logo.setImageResource(R.drawable.malawihub_logo);
        logo.setContentDescription("MalawiHub logo"); splash.addView(logo,new LinearLayout.LayoutParams(150,150));
        TextView brand = text("MalawiHub",30,Color.rgb(17,17,17)); brand.setTypeface(null,android.graphics.Typeface.BOLD);
        LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,-2); bp.topMargin=18; splash.addView(brand,bp);
        TextView portal=text("STUDENT PORTAL",15,Color.rgb(233,29,37)); portal.setTypeface(null,android.graphics.Typeface.BOLD); splash.addView(portal,new LinearLayout.LayoutParams(-1,-2));
        TextView status=text("Secure Online Class student access",14,Color.DKGRAY); LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(-1,-2); sp.topMargin=28; splash.addView(status,sp);
